...
export const Controller = <T extends object>(target: Constructable<T>): Constructable<T> => {
	/**
	 * Traps the class instantiation, so when an instance of the class is created, we can add
	 * another trap on property accessor.
	 */
	const proxyTarget: Constructable<T> = new Proxy(target, {
		construct(concreteController: Constructable<T>, argumentsArray: Array<any>): T {
